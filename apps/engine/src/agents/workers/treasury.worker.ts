...
export class TreasuryWorker {
  ...
  public start(intervalMs: number = 15 * 60 * 1000) {
    ...
    console.info(
      `🧹 [Sweeper] Treasury Worker Started. Scanning every ${intervalMs / 60000} minutes.`,
    );
    ...
    try {
      ...
      for (const account of accounts) {
        ...
        if (triggerSellReason) {
          ...
          console.info(
            `🧹 [Sweeper] HOLDING: ${mint} (Value: ${estimatedSolValue.toFixed(4)} SOL). Condition not met.`,
          );
        }
      }
    } catch (error) {
  ...
  private async executeDailyUsdcSweep(treasuryPubkey: PublicKey) {
    ...
    try {
      ...
      if (balanceSOL > 1.5) {
        ...
        console.info(
          `🏦 [Treasury:USDC] Initiating DAILY SWEEP of ${sweepAmountSOL.toFixed(2)} SOL to USDC Vault.`,
        );
        ...
        if (result.status === 'success') {
          console.info(
            `💵 [Treasury:USDC] SUCCESS! Locked SOL profit into USDC. Tx: ${result.signature}`,
          );
