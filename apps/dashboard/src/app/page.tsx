...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (authenticated) {
      ...
      setApiKeys([
        { name: 'Production - Trading Bot', key: 'sk_live_...x429', status: 'Active' },
      ]);
    }
  }, [authenticated]);
  ...
  const handleGenerateKey = async () => {
    ...
    setApiKeys([...apiKeys, { name: newKeyName || 'New API Key', key: `${mockKey.substring(0, 10)}...`, status: 'Active' }]);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[80vh] text-center px-4">
        ...
        <p className="text-slate-400 max-w-md mb-10 text-lg leading-relaxed">
          Unlock the RZUNA Alpha Intelligence via our production-grade SDK.
          Experience the power of the AIVO Protocol.
        </p>
        <button
          onClick={login}
  ...
  return (
    <div className="space-y-10 max-w-6xl mx-auto pb-20">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-100 mb-2">Developer Overview</h1>
          <p className="text-slate-400">Monitoring your B2B integration across the AIVO Protocol.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: 'API Calls (24h)', value: usageStats.apiCalls, icon: Zap, color: 'text-yellow-500', bg: 'bg-yellow-500/10' },
          { label: 'AI Credits Remaining', value: usageStats.credits, icon: ShieldCheck, color: 'text-blue-500', bg: 'bg-blue-500/10' },
          { label: 'B2B Trading Volume', value: usageStats.volume, icon: TrendingUp, color: 'text-green-500', bg: 'bg-green-500/10' },
        ].map((stat, i) => (
          <div key={i} className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
        ...
        <div className="bg-green-500/10 border border-green-500/20 p-6 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4 animate-in fade-in slide-in-from-top-4 duration-500">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm">Copy this key now. You won't be able to see it again.</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl overflow-hidden relative">
          ...
          <p className="text-slate-400 mb-8 max-w-md text-sm">
            Unlock 0.5% trading fees, unlimited AI credits, and dedicated Jito bundling for institutional performance.
          </p>
          ...
          <div className="mt-6 flex items-center gap-4 text-xs text-slate-500">
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-blue-500" /> Lowest Fees</span>
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-blue-500" /> High Rate Limit</span>
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-blue-500" /> Jito Access</span>
          </div>
        </div>
        ...
        <div className="bg-blue-600/5 border border-blue-600/20 p-8 rounded-2xl">
          ...
          <div className="bg-slate-950 p-6 rounded-xl font-mono text-sm leading-relaxed border border-slate-800 overflow-x-auto shadow-inner">
            <span className="text-blue-400">import</span> {"{ AivoClient }"} <span className="text-blue-400">from</span> <span className="text-green-400">'@rzuna/sdk'</span>;
            <br/><br/>
            <span className="text-slate-500">// Initialize Institutional Nerve</span>
            <br/>
            <span className="text-purple-400">const</span> client = <span className="text-purple-400">new</span> <span className="text-yellow-400">AivoClient</span>({"{ apiKey: '...' }"});
            <br/><br/>
            <span className="text-slate-500">// Subscribe to High-Alpha Signals</span>
            <br/>
            client.<span className="text-yellow-400">on</span>(<span className="text-green-400">'signal'</span>, (signal) {"=>"} console.<span className="text-yellow-400">log</span>(signal));
          </div>
          <div className="mt-6 flex justify-between items-center text-xs">
            <p className="text-slate-500">Read the <a href="#" className="text-blue-400 hover:text-blue-300 underline underline-offset-4">Full SDK Documentation</a></p>
      ...
      {showKeyModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-2xl p-8 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-slate-400 text-sm mb-6">Give your key a descriptive name to track usage easily.</p>
            
            <div className="space-y-4 mb-8">
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block">Key Name</label>
                <input 
                  type="text" 
                  value={newKeyName}
