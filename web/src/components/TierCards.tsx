      ...
      {Tiers.map((tier) => (
        <motion.div
          ...
          className={`relative overflow-hidden bg-[#11111a] border ${
            profile?.subscription_status === tier.id ? 'border-purple-500 shadow-[0_0_30px_rgba(168,85,247,0.15)]' : 'border-white/5'
        ...
        >
          ...
          <button
            ...
            disabled={profile?.subscription_status === tier.id}
          ...
          >
            {profile?.subscription_status === tier.id ? 'Current Plan' : 'Secure Access'}
          </button>
        </motion.div>
      ))}
    </div>
