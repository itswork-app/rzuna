...
export function TelegramSettings({ profile, onUpdate }: TelegramSettingsProps) {
  ...
  const [chatId, setChatId] = useState(profile?.tg_chat_id || '');
  const [isEnabled, setIsEnabled] = useState(profile?.is_tg_enabled || false);
  ...
  const isPremium = profile?.subscription_status &&
    ['STARLIGHT', 'STARLIGHT_PLUS', 'VIP'].includes(profile.subscription_status);
