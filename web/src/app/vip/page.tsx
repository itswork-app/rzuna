...
export default function VIPChannel() {
  ...
  const handleSubscribe = async (tier: string, priceUSDC: number) => {
    ...
    try {
      ...
      console.error(err);
      alert('Subscription failed. Ensure you have sufficient USDC for institutional settlement.');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6 font-sans selection:bg-purple-500/30">
      ...
      {/* 🏛️ PR 18: Institutional Settlement via USDC */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {[
            ...
            >
              ...
              >
                {isSubscribing === tier.id ? 'Settling...' : profile?.subscription_status === tier.id ? 'Active' : 'Upgrade to USDC'}
              </button>
            </motion.div>
          ))}
        </div>
